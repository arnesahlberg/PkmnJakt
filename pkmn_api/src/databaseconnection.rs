...
pub fn get_catches_per_day(datetime0: Option<&str>, datetime1: Option<&str>, conn: &Connection) -> Result<Vec<crate::model::DailyCatchStats>> {
    let mut query = String::from( // should be a view up ahead XXXX
        "SELECT 
            ...
            CAST(strftime('%d', found_timestamp) AS INTEGER) as day_number,
            COUNT(*) as count
         FROM FoundPokemon
         WHERE user_id != 'admin'"
    );
    ...
    query.push_str(" GROUP BY date ORDER BY date");
    
    let mut stmt = conn.prepare(&query)?;
    let params: Vec<&dyn rusqlite::ToSql> = params_vec.iter().map(|p| p as &dyn rusqlite::ToSql).collect();
    
    let daily_stats = stmt.query_map(&params[..], |row| {
        let weekday_num: i32 = row.get(1)?;
        ...
        Ok(crate::model::DailyCatchStats {
            date: row.get(0)?,
