...
class _GameStatisticsScreenState extends State<GameStatisticsScreen> {
  ...
  Future<void> _loadStatistics() async {
    ...
    try {
      final stats = await ApiService.getGameSummaryStatistics();
      if (mounted) {
        setState(() {
          _statistics = stats;
          _isLoading = false;
          ...
          PokedexContainer(
            child: Column(
              ...
              children: [
                ...
                _buildStatRow('Totalt antal deltagare', 
                    '${_statistics!['total_users_registered'] ?? 0}'),
                const Divider(),
                _buildStatRow('Spelare med 10+ fångster', 
                    '${_statistics!['users_with_10_plus_catches'] ?? 0}'),
                const Divider(),
                _buildStatRow('Spelare med 100+ fångster', 
            ...
            PokedexContainer(
              child: Column(
                ...
                children: [
                  ...
                  if (_statistics!['first_catch'] != null) ...[
                    _buildCatchInfo('Första fångsten', _statistics!['first_catch']),
                    if (_statistics!['last_catch'] != null) const Divider(),
                  ],
                  if (_statistics!['last_catch'] != null)
                    _buildCatchInfo('Sista fångsten', _statistics!['last_catch']),
                ],
              ),
            ),
            const SizedBox(height: 16),
  ...
  Widget _buildPokemonStatsList(List<dynamic> pokemonStats) {
    return Column(
      children: pokemonStats.take(10).map((stat) {
        return Padding(
          ...
          Expanded(
            child: Row(
              ...
              children: hourlyData.map<Widget>((data) {
                ...
                final heightFactor = maxCatches > 0 ? catches / maxCatches : 0.0;
